...
#if defined(NDEBUG)
# error "gAsp cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "gAsp Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (tx.IsCoinBase())
    {
        ...
        if(chainParams.GetConsensus().hashGenesisBlock != Params(CBaseChainParams::TESTNET).GenesisBlock().GetHash() && 
            chainParams.GetConsensus().hashGenesisBlock != Params(CBaseChainParams::REGTEST).GenesisBlock().GetHash()) {
            ...
            if(tx.vout[0].scriptPubKey != genesisOutputScript) {
                ...
                if (tx.nVersion == 1)
                    vout = 1;
                CTxDestination curAddress = CBitcoinAddress(chainParams.GetConsensus().associationAddress).Get();
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (mempoolRejectFee > 0 && nModifiedFees < mempoolRejectFee) {
            return state.DoS(0, false, REJECT_INSUFFICIENTFEE, strprintf("mempool min fee not met, %d < %d", nFees, mempoolRejectFee), false, strprintf("%d < %d", nFees, mempoolRejectFee));
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s GASP additional fees, %d delta bytes\n",
...
{
    if(consensusParams.hashGenesisBlock == Params(CBaseChainParams::TESTNET).GenesisBlock().GetHash()) {
        // Testnet
        return COIN;
    }

    if(consensusParams.hashGenesisBlock == Params(CBaseChainParams::REGTEST).GenesisBlock().GetHash()) {
        // Regtest
        return COIN;
    }
    ...
    int existing_coins = (1000000000 * COIN) + ((nHeight-1) * 0 * COIN);
    int total_coins = 1000000000 * COIN;

    if(existing_coins >= total_coins)
        return 0;

    if(existing_coins + (0 * COIN) > total_coins)
        return total_coins - existing_coins;
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW)
{
    ...
    if (ibdComplete && fCheckPOW && !CheckProofOfWork(block.GetPoWHash(), block.nBits, consensusParams))
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (!fReindex && (nHeight >= consensusParams.BIP34Height))
    {
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if(!fReindex && !CheckSyncCheckpoint(hash, pindexPrev))
...
bool static LoadBlockIndexDB(const CChainParams& chainparams)
{
    ...
    // AspireGas: load hashSyncCheckpoint
    if (!acpdb->ReadACP(hashSyncCheckpoint)) {
        LogPrintf("LoadBlockIndexDB(): acp not read, using genesis block\n");
        hashSyncCheckpoint = chainparams.GetConsensus().hashGenesisBlock;
    } else
        LogPrintf("LoadBlockIndexDB(): acp read at %s\n", hashSyncCheckpoint.ToString().c_str());
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // gAsp: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
